...
CUpDownClient::~CUpDownClient(){
	...
	if( theApp.natthread /*&& this peer is NatTrans Peer*/ )
		theApp.natthread->RemoveNatSocket( m_achUserHash );
...
bool CUpDownClient::Disconnected(LPCTSTR pszReason, bool bFromSocket)
{
	...
	if( m_nKadState==KS_CONNECTING_BUDDY )
	...
	switch(m_nUploadState){
		...
		case US_ERROR:
			if (bAddDeadSource && m_iErrTimes>6)
				theApp.clientlist->m_globDeadSourceList.AddDeadSource(this);
			bDelete = true;
			m_iErrTimes++;
	}
...
bool CUpDownClient::TryToConnect(bool bIgnoreMaxCon, CRuntimeClass* pClassSocket){
	...
	if ( HasLowID() && (GetSourceFrom() != SF_LAN)) // VC-kernel[2007-01-17]:
	{
...
#ifndef _DEBUG
			if(m_nUserPort!=0xffff && m_nSourceFrom!=SF_SERVER && m_bSupportNatTraverse==false)
			{
...
#endif
			if(m_nUserPort==0xffff) m_bSupportNatTraverse = true;
		...
		if( HasValidBuddyID() && !GetBuddyIP() && !GetBuddyPort() && !theApp.serverconnect->IsLocalServer(GetServerIP(), GetServerPort()))
		{
			...
			// VC-SearchDream[2007-04-09]: For Source Exchange Call Back
			if (m_nSourceFrom == SF_SOURCE_EXCHANGE && m_pSourceExchangeClient != NULL)
			{
			...
			// VC-SearchDream[2007-04-09]: For Source Exchange Call Back End
...
CString CUpDownClient::GetDownloadStateDisplayString() const
{
	...
	switch (GetDownloadState())
	{
		...
		case DS_LOWTOLOWIP:
			...
			if( !m_bSupportNatTraverse )// VC-Huby[2007-01-05]: 需优先判断m_bSupportNatTraverse
			{
				if(m_nUserPort!=0xffff || m_nSourceFrom==SF_SERVER)
				{
