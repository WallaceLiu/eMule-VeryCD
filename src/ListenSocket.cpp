...
bool CClientReqSocket::CheckTimeOut()
{
	if(m_nOnConnect == SS_Half)
	{
		...
		if(theApp.natthread && theApp.natthread->GetUsedTime(this))
		{
			timeout_timer = ::GetTickCount();
			return false;
		}
	...
	if (::GetTickCount() - timeout_timer > uTimeout){
		...
		if(theApp.natthread && m_bUseNat) theApp.natthread->RemoveSocket(this);
		return true;
	}
	return false;
}
...
void CClientReqSocket::Safe_Delete()
{
	...
	theApp.uploadBandwidthThrottler->RemoveFromAllQueues(this);//ADDED by VC-fengwen on 2007/09/24 : 尝试解决EMSocket的一些Crash：此时应清空待发数据，再等上几秒，之后再delete，应该不会有错了。
}
...
{
	timeout_timer = ::GetTickCount();
	if(client) client->SetDownloadState(DS_LOWTOLOWIP);
}
