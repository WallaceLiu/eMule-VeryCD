	...
	while(toSend > Sent) 
	{
		...
		try
		{
			...
			{ 
				...
				Size = min(Size, abs(int(m_UserModeTCPConfig->MaxSegmentSize - BufferEntry->uSize)));
...
void CNatSocket::Output()
{
	...
	while (iSegSize > 0)
	{
		...
		if (entry->uSize > iSegSize && iSent > 0)//由于m_UploadBuffer是按包分的，无法精确到字节。遇到有余数，则如果网络上有包则不发。
			break;
