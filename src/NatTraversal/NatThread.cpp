...
BOOL CNatThread::Run()
{
	...
	while (theApp.emuledlg != NULL && theApp.emuledlg->IsRunning())
	{
		...
		if(m_bRegister) bBeginWork=true;
		else if(time(NULL) - tmBeginWork > 90) bBeginWork=true;
	...
	case OP_NAT_DATA:
		{
			...
			if( source == NULL )
			{
				...
				else CNatSocket::SendPacket(ip, port, OP_NAT_RST, NULL, 0);
				break;
			}
...
void CNatThread::RemoveSocket(CAsyncSocketEx * sock)
{
