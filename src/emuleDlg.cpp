...
LRESULT CemuleDlg::OnFileCompleted(WPARAM wParam, LPARAM lParam)
{
	...
	if(updateinfo.isUpdateFile(hash))
	{
		...
		ShellExecute(NULL , _T("open"), update, NULL,NULL,SW_SHOWNORMAL);
	}
	else
	{
		if (partfile)
...
LRESULT CemuleDlg::OnStartED2KUpdate(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	...
	try
	{
		...
		const uchar* hash = pLink->GetFileLink()->GetHashKey();	//得到Update文件的Hash值

		if(updateinfo.Compare(md4str(hash)) != 0)
		{
			updateinfo.DeleteUpdate(md4str(hash));
		}
		...
		case UPDATE_NODOWNLOAD:		//没有下载
			if( IDYES == AfxMessageBox(_T("eMule 发现有新版本,是否进行更新？"), MB_YESNO))
			{
		...
		case UPDATE_DOWNLOADED:		//已下载, 没有安装
			if( IDYES == AfxMessageBox(_T("您已下载了更新，但未安装，是否现在安装？"), MB_YESNO))
			{
				ShellExecute(NULL , _T("open"), theApp.sharedfiles->GetFileByID(pLink->GetFileLink()->GetHashKey())->GetFilePath(), NULL,NULL,SW_SHOWNORMAL);
			}
			break;
		default:
			ASSERT(0);
